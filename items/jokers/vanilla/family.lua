SMODS.Joker({
	...
	calculate = function(self, card, context)
		if --The card transformation apspect of this was taken and modifed in part from the Waterfall Joker from the Celesete Card Collection
			context.before and context.poker_hands ~= nil and next(context.poker_hands["Four of a Kind"])
		then
			local rank_count = {}
		...
		if (context.joker_main and context.poker_hands ~= nil and next(context.poker_hands["Four of a Kind"])) or context.forcetrigger then
			return {
				message = localize({
					type = "variable",
					key = "a_powmult",
