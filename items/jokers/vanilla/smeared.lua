...
function is_id(card, ids)
    if next(SMODS.find_card("j_asc_smeared")) then return true end
    if SMODS.has_no_rank(card) and not card.vampired then return false end
    for _, value in ipairs(ids) do
        if card:get_id() == value then return true end
    end
    return false
end
...
function Card:get_id()
    if self.playing_card and next(SMODS.find_card("j_asc_smeared")) then return 14 end
    return oldgetid(self)
end

local oldissuit = Card.is_suit
function Card:is_suit(suit, bypass_debuff, flush_calc)
    if self.playing_card and next(SMODS.find_card("j_asc_smeared")) then return true end
...
SMODS.current_mod.set_debuff = function(card)
    if next(SMODS.find_card("j_asc_smeared")) and card.playing_card and (not SMODS.has_no_rank(card) or not SMODS.has_no_suit(card)) then
...
function ids_op(card, op, b, c)
    ...
    local function alias(x)
        ...
        if card.playing_card and has_smeared then return 11 end
        return x
    end
