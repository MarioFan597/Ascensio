SMODS.Joker({
    ...
    config = {
        extra = {
            ...
            immutable = { max_retriggers = 100, slots = 0 },
        },
    },
    ...
    loc_vars = function(_, _, card)
        return {
            vars = {
                math.min(lenient_bignum(card.ability.extra.retriggers), card.ability.extra.immutable.max_retriggers),
                lenient_bignum(card.ability.extra.immutable.max_retriggers),

                card.ability.extra.slot_gain,
                card.ability.extra.immutable.slots,
    ...
    calculate = function(self, card, context)
        if (context.ending_shop or context.forcetrigger) and not context.blueprint then
            ...
            ease_joker_slot(card.ability.extra.slot_gain)
