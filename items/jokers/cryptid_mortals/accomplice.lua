    ...
    calculate = function(self, card2, context)
        if (context.end_of_round and not context.repetition and not context.individual) or context.forcetrigger then
            ...
            then
                ...
                check = true
            elseif (card.config.center.effect == "Type Mult" or card.config.center.effect == "Cry Type Mult" or card.config.center.effect == "Boost Kidnapping") and not Card.no(G.jokers.cards[1], "immutable", true) then
