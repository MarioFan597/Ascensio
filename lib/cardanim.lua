...
G.E_MANAGER:add_event(Event({
	...
	func = function() -- anti-nesting
		...
		if G.cardanim.animation_details then
			return true
		end
		...
		function Game:update(dt)
			...
			for card_key, card_anim_d in pairs(anim_details) do
				...
				for kw, part in pairs(G.cardanim.card_layers) do
					if frame_seq[kw] and part.coords(card_def) then
