...
function G.UIDEF.use_and_sell_buttons(card)
	...
	if (card.area == G.jokers and G.jokers and card.config.center.key == "j_asc_marble_entr") and not card.debuff then --Gives buttons to entropic marble Joker
	        sell = {n=G.UIT.C, config={align = "cr"}, nodes={ --Default Sell Button
	        ...
	        buyslot = {n=G.UIT.C, config={align = "cr"}, nodes={ --Button to Buy Stone Cards
	            {n=G.UIT.C, config={ref_table = card, align = "cr",padding = 0.1, r=0.08, minw = 1.25, hover = true, shadow = true, colour = G.C.UI.BACKGROUND_INACTIVE, button = 'buy_stone', func = 'can_buy_stone'}, nodes={
	              {n=G.UIT.B, config = {w=0.1,h=0.6}},
	              {n=G.UIT.C, config={align = "tm"}, nodes={
	                {n=G.UIT.R, config={align = "cm", maxw = 1.25}, nodes={
	                  {n=G.UIT.T, config={text = localize('b_buy_stone'),colour = G.C.UI.TEXT_LIGHT, scale = 0.4, shadow = true}}
	                }},
	                {n=G.UIT.R, config={align = "cm"}, nodes={
	                  {n=G.UIT.T, config={text = localize('$'),colour = G.C.WHITE, scale = 0.4, shadow = true}},
	          ...
	          sellslot = {n=G.UIT.C, config={align = "cr"}, nodes={ --Button to Sell Stone Cards
	            {n=G.UIT.C, config={ref_table = card, align = "cr",padding = 0.1, r=0.08, minw = 1.25, hover = true, shadow = true, colour = G.C.UI.BACKGROUND_INACTIVE, button = 'sell_stone', func = 'can_sell_stone'}, nodes={
	              {n=G.UIT.B, config = {w=0.1,h=0.6}},
	              {n=G.UIT.C, config={align = "tm"}, nodes={
	                {n=G.UIT.R, config={align = "cm", maxw = 1.25}, nodes={
	                  {n=G.UIT.T, config={text = localize('b_sell_stone'),colour = G.C.UI.TEXT_LIGHT, scale = 0.4, shadow = true}}
	                }},
	                {n=G.UIT.R, config={align = "cm"}, nodes={
	                  {n=G.UIT.T, config={text = localize('$'),colour = G.C.WHITE, scale = 0.4, shadow = true}},
...
G.FUNCS.can_buy_stone = function(e)
    if
        to_big(G.GAME.dollars-G.GAME.bankrupt_at) >= to_big(e.config.ref_table.ability.buycost) and not is_buying_stone and not G.CONTROLLER.locked
    then
...
G.FUNCS.can_sell_stone = function(e)
    ...
    if
        has_stone and not G.CONTROLLER.locked
    then
        e.config.colour = G.C.GREEN
